...
jQuery( function( $ ) {
    ...
    (add_events = function( object )
    {
        ...
        // Accordion
        $('.js-cuztom-accordion', object).accordion({ heightStyle: 'content' });
    ...
    $('.cuztom').on( 'click', '.js-cuztom-add-sortable', function()
    {
        ...
        /**
         * Marcus Gullberg - 2015
         * There is a bug on the cuztom framework that it just takes the ID of your last bundle and then increment
         * that ID by one when it adds the new metabox. Maybe you have sorted you bundles so the bundle with the
         * last ID is not the last box. That way you will have overwrites ID's and it will not work.
         * This fix will loop through all of you existing boxes and find the one with the HIGEST ID and clone that one
         * Thay way when we clone the new one it will not overwrite an existing key but it will create a unique one.
         *
         * Bug url: https://github.com/gizburdt/wp-cuztom/issues/336
         */
        var last;
        var lastID = 0;
        var items = wrap.find('.js-cuztom-sortable-item');

        // Loop through the items
        if (items) {
            items.each(function() {
                var select = $(this).find('.cuztom-select');
                if (select.length) {
                    var id = parseInt(select.attr('name').match(/\[(\d+)\]/)[1]);
                    if(id > lastID) {
                        lastID = id;
                        last   = $(this);
                    }
        ...
        // Clone the item that we found with the highest ID
        var new_item = last.clone( false, false );
        ...
        if( is_bundle )
        {
            new_item.find('tr').each(function() {
                ...
                if( cuztom_input.hasClass('wp-editor-area' )) {
                    ...
                    /**
                     * Jesse Pasos - 2015
                     * Bug fix for having multiple wysiwyg editors in a cuztom bundle
                     */
                    ...
                    {   
                        ...
                        QTags_options = { id: new_id, buttons: QTags_last.settings.buttons, length: 1 };
