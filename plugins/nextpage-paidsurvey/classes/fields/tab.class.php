			...
			else
			{
				echo '<table border="0" cellpadding="0" cellspacing="0" class="form-table cuztom-table">';
					foreach( $fields as $id => $field )
					{
						...
						// Get the input value based on meta type, default = post meta
						switch ($this->meta_type) {
							case 'user':
								$value = get_user_meta( $post->ID, $id, true );
								break;

							case 'option':
								$option = get_option( $field->parent );
								$value = !empty( $option[$field->id] ) ? $option[$field->id] : '';
								break;
