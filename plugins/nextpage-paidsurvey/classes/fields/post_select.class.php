	...
	function _output( $value )
	{
		$output = '<select ' . $this->output_name() . ' ' . $this->output_id() . ' ' . $this->output_css_class() . '>';
			...
			if( is_array( $this->posts ) )
			{
				...
				{
					// Default label
					$strLabel = $post->post_title;
				
					// If we're getting multiple post types prefix it with the post type
					if (is_array($this->args['post_type'])) {
						$strLabel = Cuztom::beautify($post->post_type) . ' - ' . $strLabel;
					}

					$output .= '<option value="' . $post->ID . '" ' . ( ! empty( $value ) ? selected( $post->ID, $value, false ) : selected( $this->default_value, $post->ID, false ) ) . '>' . $strLabel . '</option>';
				}
			}
		$output .= '</select>';
